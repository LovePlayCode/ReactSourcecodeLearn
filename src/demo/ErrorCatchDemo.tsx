...
const {
  log,
  getPhaseFromExecutionContext,
  COLOR: { SCHEDULE_COLOR, RENDER_COLOR, COMMIT_COLOR, USERSPACE_COLOR },
} = utils;
...
bindHook(
  ...
  (fnName, executionContext, fiber, update) => {
    log(
      phaseName2Color[getPhaseFromExecutionContext(executionContext)],
      `由于${fnName}，在ClassComponent创建ErrorUpdate`,
      fiber
    );
  }
);
...
bindHook("ErrorUpdate on ClassComponent", (phase: Phase, update, fiber) => {
  log(
    phaseName2Color[phase],
    `${phase}阶段在ClassComponent调度ErrorUpdate`,
    fiber
  );
});
